...
export function getResidentialTariff(voltageType: VoltageType, season: Season): ResidentialTariff {
  ...
  return {
    ...
    notes: [
      ...
      '기후환경요금과 연료비조정단가는 변동될 수 있어 운영 시 공식 요금표 또는 서버 설정값으로 갱신해야 합니다.',
...
export const CIVIL_SERVICE_ITEMS: KepcoCivilServiceItem[] = [
  ...
  civilItem('self_meter_reading', '자가검침 지침입력', '검침 및 납기일', 'metering_or_due_date', 'needs_user_auth_or_api', '자가검침 계량기 지침을 입력합니다.', ['자가검침', '지침입력', '직접 검침', '검침 입력', '자가검침 계량기']),
  ...
  civilItem('meter_inspection_exchange', '전력량계 점검 및 교환신청', '전기설비', 'metering_or_due_date', 'needs_user_auth_or_api', '전력량계 점검 또는 교환을 신청합니다.', ['전력량계', '계량기 점검', '계량기 교환', '계량기 숫자', '검침값 이상', '검침값 확인', '지침 이상', '숫자 이상', '사용량 이상', '계량기 확인 요청']),
