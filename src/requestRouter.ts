...
function detectIntents(text: string): IntentCandidate[] {
  ...
  const hasCivilWord = hasAny(text, [
    /한전|한전ON|민원|신청|명의|이사\s*정산|전기사용신청|증설|계약전력|자동이체|청구서|고객번호|복지할인|정전|전기고장|서류|신청서|양식|FAQ|자주\s*묻|해지|사용\s*중지|전기\s*끊|폐업|휴지|미납/
  ]);
  const hasCivilActionWord = hasAny(text, [
    /민원|신청|명의|이사\s*정산|전기사용신청|증설|계약전력|자동이체|청구서|고객번호|복지할인|정전|전기고장|서류|신청서|양식|FAQ|자주\s*묻|제출|조회|해지|사용\s*중지|전기\s*끊|폐업|휴지|미납/
  ]);
